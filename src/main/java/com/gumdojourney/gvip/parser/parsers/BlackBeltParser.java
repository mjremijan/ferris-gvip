...
public class BlackBeltParser implements FilenameParser {
    private static final Pattern PAT = Pattern.compile("Black belt (\\d+(st|nd|rd|th)) Dan (\\d+(st|nd|rd|th)) self-defense form .*\\((\\d{4}-\\d{2}-\\d{2}) .*\\)\\.mp4", Pattern.CASE_INSENSITIVE);
