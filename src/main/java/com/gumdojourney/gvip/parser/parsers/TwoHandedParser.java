...
public class TwoHandedParser implements FilenameParser {
    ...
    public Optional<Metadata> parse(Path filePath, String filename) {
        ...
        md.setTitle(String.format("Two-Handed #%s Sword Form (%s)%s- Haidong Gumdo", num, date.substring(0, 7), hasCommentary ? " Commentary " : " "));
        md.setDescription(filename);
        md.setRecordingDate(LocalDate.parse(date, DATE));
        md.setPlaylists(
            Arrays.asList(String.format("Two-Handed #%s Sword Form", num), hasCommentary ? "Commentary" : null)
                .stream()
